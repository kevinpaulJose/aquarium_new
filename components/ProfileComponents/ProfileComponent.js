...
class ProfileComponent extends React.Component {
  ...
  _logOutComponent = () => (
    <TouchableOpacity
      activeOpacity={0.8}
      onPress={() => {
        Alert.alert("Log Out", "Are you sure you need to Log out?", [
          {
            text: "Log out",
            onPress: () => {
              firebase
  ...
  _renderOrder = () => (
    ...
    >
      <View style={{ marginTop: 20, alignItems: "flex-start", marginLeft: 10 }}>
        <Text style={{ fontSize: 18, color: theme.primaryDark }}>
          Your Orders
        </Text>
      </View>
  ...
  _renderOrderCard = ({ item }, { key }) => (
    <View key={key} style={{ marginTop: 10 }}>
      ...
      >
        <Icon
          ...
          onPress={() => {
            console.log(item.id);
            let messageURL =
              "https://wa.me/%2B919442964818?text=Hi,%20I%20need%20help%20with%20this%20order%20" +
              item.id;
      ...
      {!item.address.servicable && (
        ...
        >
          This order may not be servicable.
        </Text>
      )}
      {item.products.map((i, k) => {
        ...
        return (
          ...
          >
            <View
              style={{ width: 50, height: 50, marginTop: 5, marginLeft: 5 }}
            >
              <Image
                source={{ uri: i.image }}
                style={{
            ...
            <View style={{ flexDirection: "column", marginLeft: 5 }}>
              ...
              <View style={{ marginTop: 5, marginLeft: 5 }}>
                <Text style={{ color: theme.lightTextColor }}>
                  x {i.number}
                </Text>
              </View>
            </View>
      ...
      <Text
        style={{ color: theme.primaryDark, marginTop: 10, textAlign: "right" }}
      >
        Status: {item.status}
      </Text>
    </View>
      ...
      <ScrollView>
        ...
        >
          {this.props.userSystemData.data.length > 0 && (
            <Text style={{ fontSize: 20, color: theme.primaryDark }}>
              Hello {this.props.userSystemData.data[0].username}!
            </Text>
          )}
        </View>
