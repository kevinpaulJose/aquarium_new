...
class CartCardComponent extends React.Component {
    ...
    render() {
        let product = this.props.prodData.data.find(i => i.productId === this.props.item.productid);
        return (
            ...
            <View>
                <View style={{
                    ...
                    <View style={{flexDirection: 'row', alignItems: "center"}}>
                        <View style={{width: 50, height: 50, marginTop: 10, marginLeft: 10, borderRadius: 10}}>
                            <Image source={{uri: product.image}} style={{width: 50, height: 50, borderRadius: 10}}/>
                        </View>
                        <View style={{marginLeft: 30, flexDirection: "column"}}>
                            <Text style={{fontWeight: "bold", color: theme.primaryDark}}>{product.name}</Text>
                            <Text style={{color: theme.darkTextColor}}>₹{product.netPrice}</Text>
                        </View>
                    </View>
                    <View style={{
                        width: 70,
                        ...
                        {this.props.item.count === 1 ?
                            <TouchableOpacity activeOpacity={0.8} onPress={() => {
                                ...
                                cartCollection.where("cartid", "==", this.props.item.cartid)
                            ...
                            <TouchableOpacity onPress={() => {
                                ...
                                cartCollection.where("cartid", "==", this.props.item.cartid)
                                    ...
                                    querySnapshot.forEach((doc) => {
                                        ...
                                        cartCollection.doc(docId).set({count: count}, {merge: true}).then(() => {
                                            this.setState({loading: false})
                                            this.props.getCart(this.props.userSystemData.data[0].userid);
                                        }).catch((error) => {
                                            alert("error");
                        ...
                        <TouchableOpacity onPress={() => {
                            ...
                            <Text style={{color: theme.lightTextColor, fontWeight: "bold"}}>+</Text>
                        </TouchableOpacity>
                    </View>
